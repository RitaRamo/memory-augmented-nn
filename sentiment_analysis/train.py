...
def main():
    ...
    train_retrieval_iterator = torch.utils.data.DataLoader(
        TrainRetrievalDataset(train_sents,bert_model, DEBUG, MAX_LEN),
        batch_size=BATCH_SIZE, shuffle=False, num_workers=0
    )

    text_retrieval=TextRetrieval(train_retrieval_iterator, train_labels, MODEL_TYPE)
    ...
    #####
    train_iterator = torch.utils.data.DataLoader(
        SADataset(train_sents, train_sents_ids, train_lens, train_labels, bert_model, max_len),
        batch_size=BATCH_SIZE, shuffle=True, num_workers=0
    )

    val_iterator = torch.utils.data.DataLoader(
        SADataset(val_sents, val_sents_ids, val_lens, val_labels, bert_model, max_len),
        batch_size=BATCH_SIZE, shuffle=False, num_workers=0
    )

    test_iterator = torch.utils.data.DataLoader(
        SADataset(test_sents, test_sents_ids, test_lens, test_labels, bert_model, max_len),
        batch_size=BATCH_SIZE, shuffle=False, num_workers=0
    )
